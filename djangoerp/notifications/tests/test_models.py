...
class FollowRelationTestCase(TestCase):
    def test_unicode_representation(self):
        ...
        u1 = user_model.objects.create(username=u"u1")
        u2 = user_model.objects.create(username=u"u2")
        fr = FollowRelation.objects.create(followed=u2, follower=u1)
        
        self.assertEqual(u"%s" % fr, u"%s followed by %s" % (u2, u1))

class SignatureTestCase(TestCase):
    def test_unicode_representation(self):
        """Tests unicode representation.
        """
        title = u"Create a generic object"
        s = Signature.objects.create(slug=u"create_object", title=title)
        
        self.assertEqual(u"%s" % s, title)
        
    def test_generate_title_on_save(self):
        """Tests automatic title generation on save.
        """
        s = Signature.objects.create(slug=u"edit_object")
        
        self.assertEqual(s.title, u"Edit object")
...
class SubscriptionTestCase(TestCase):
    def test_unicode_representation(self):
        ...
        s = Signature.objects.create(slug=u"object-changed")
        sub = Subscription.objects.create(subscriber=u1, signature=s)
        
        self.assertEqual(u"%s" % sub, u"%s | %s" % (u1, s))
...
class ActivityTestCase(TestCase):
    def setUp(self):
        ...
        self.signature = u"object-changed"
        
    def test_unicode_representation(self):
        """Tests unicode representation.
        """
        a1 = Activity.objects.create(title=u"New activity %(name)s 1", source=self.u1, signature=self.signature)
        
        self.assertEqual(u"%s" % a1, u"New activity %(name)s 1")
        
        a1.context = '{"name": "with name"}'
        
        self.assertEqual(u"%s" % a1, u"New activity with name 1")
        
    def test_get_context(self):
        """Tests "get_context" method.
        """
        a2 = Activity.objects.create(title=u"Activity 2", source=self.u1, signature=self.signature)
        
        self.assertEqual(a2.get_context(), {})
        
        a2.context = '{"name": "with name"}'
        
        self.assertEqual(a2.get_context(), {u"name": u"with name"})
        
    def test_get_template_name(self):
        """Tests "get_template_name" method.
        """
        a3 = Activity.objects.create(title=u"Activity 3", source=self.u1, signature=self.signature)
...
class NotificationTestCase(TestCase):
    def setUp(self):
        ...
        self.u1 = user_model.objects.create(username=u"u1")
        self.signature = Signature.objects.create(slug=u"object-changed")
        self.n1 = Notification.objects.create(title=u"Notification 1", target=self.u1, signature=self.signature)
        
    def test_unicode_representation(self):
        """Tests unicode representation.
        """
        self.assertEqual(u"%s" % self.n1, u"Notification 1")
        
    def test_get_urls(self):
        """Tests "get_*_url" methods.
        """        
        self.assertEqual(self.n1.get_absolute_url(), u"/users/%s/notifications/%s/" % (self.u1.pk, self.n1.pk))        
        self.assertEqual(self.n1.get_delete_url(), u"/users/%s/notifications/%s/delete/" % (self.u1.pk, self.n1.pk))
...
class ObservableTestCase(TestCase):
    ...
    def test_caching_changes(self):
        ...
        """
        self.u1.username = u"u1"
        self.u1._Observable__changes = {}
        
        self.u1.username = u"u2"
        self.assertEqual(self.u1._Observable__changes, {"username": (u"u1", u"u2")})
        
        self.u1.username = u"u3"
        self.assertEqual(self.u1._Observable__changes, {"username": (u"u1", u"u3")})
...
class NotificationTargetTestCase(TestCase):
    ...
    def test_notification_set(self):
        ...
        """
        n1 = Notification.objects.create(title=u"Notification 1", target=self.u1, signature=self.signature)
        n2 = Notification.objects.create(title=u"Notification 2", target=self.u1, signature=self.signature)
